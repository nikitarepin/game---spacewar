...
def events(screen, gun, bullets):
    for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                #движение вправо при удержании клавиши
                if event.key == pygame.K_RIGHT:
                    gun.mright = False
                #влево
                elif event.key == pygame.K_LEFT:
...
def update_bullets(screen, stats, sc, inos, bullets):
    #обновляет позиции пуль чтобы их удалять когда они выходят за пределы
    bullets.update()
...
def gun_kill(stats, screen, sc,  gun, inos, bullets):
    #столкнвение пушки и пришельцев
    if stats.gun_left > 0:
        stats.gun_left -= 1 #отняли жизнь
        sc.image_guns() #отрисовали жизнь
        inos.empty()
        bullets.empty()
        create_army(screen, inos)
        gun.create_gun()
